...
**参考效果**（GPU, 2000 样本 15 epoch）：MCM loss 0.93 → **0.245**（约 2.5 分钟；GPU 前向加速比约 17×）。
...
**参考效果**（GPU, 2400 样本 40 epoch）：val BCE 0.674 → **0.257**，valBER 0.419 → **0.115**（约 8.5 分钟/模型）。
...
|-----|---------|-------------|------|-----------|
| -5 dB | 0.3974 | 0.3482 | **-12.4%** | 0.3380 |
| 0 dB | 0.3397 | 0.2540 | **-25.2%** ✅ | 0.2502 |
| 5 dB | 0.2286 | 0.1429 | **-37.5%** ✅ | 0.1374 |
| 10 dB | 0.1483 | 0.0987 | **-33.4%** ✅ | 0.1032 |
| 15 dB | 0.1201 | 0.0310 | **-74.2%** ✅ | 0.0385 |
| 20 dB | 0.0564 | 0.0072 | **-87.2%** ✅ | 0.0108 |

### 8.2 按配置维度（BER, 全 SNR 平均）

| 维度 | lwm vs base | 结论 |
|------|-------------|------|
| 天线 1/2/4/8 | 0.223/0.138/0.145/0.083 vs 0.291/0.185/0.216/0.168 | 全部改善（-22%~-51%） |
| RB 1~10 | 1RB 0.174 vs 0.236，4RB 0.115 vs 0.197，10RB 0.138 vs 0.186 | 全部改善 |
| 符号 3~14 | 3symb 0.079 vs 0.162，7symb 0.175 vs 0.232，14symb 0.101 vs 0.185 | 全部改善 |
| DMRS {1}/{1+1}/{1+2} | 0.154/0.153/0.127 vs 0.225/0.221/0.191 | 全部改善 |
| TDL-A/B/C/D | 0.223/0.138/0.145/0.083 vs 0.291/0.185/0.216/0.168 | 全部改善 |
| 速度 0/5/30 m/s | 0.154/0.153/0.127 vs 0.225/0.221/0.191 | 全部改善（含 350Hz 多普勒） |

### 8.3 结论

- **一个模型适配所有系统参数**：天线 1/2/4/8 × RB 1~10 × 符号 3~14 × DMRS 三模式 × TDL-A/B/C/D × 时延/多普勒，-5~20 dB 全区间 BER 改善 **12.4%~87.2%**，所有配置维度无劣化。
...
- **高 SNR 增益最大**（15~20dB ~ -74%~-87%）：模型学会了各配置下对 LS 信道估计误差的系统性修正。
- **继续预训练 vs 官方权重**：两者相当（对照在低 SNR 略优，主模型在高 SNR 略优；骨干 lr=1e-6 过小，见 §11）。
...
**Q6：为什么继续预训练收益不明显？**
当前 MCM（30 epoch, loss 0.426）与官方权重对照的差异 <0.5%（当前训练），主要因为骨干微调 lr=1e-6 过小。改进建议：提高骨干 lr（1e-4 量级）或两阶段训练（先冻骨干训 decoder，再联合微调），见第 11 节。
...
1. **解决继续预训练价值问题**：对照（无 MCM 预训练）在部分 SNR 反而略优——骨干微调 lr=1e-6 过小，建议提高骨干 lr（1e-4 量级）或两阶段训练（先冻骨干训 decoder，再联合微调）。

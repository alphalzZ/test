...
本项目复用官方 LWM 权重作为骨干初始化（`src/models/lwm_llr.py` 的 `load_official_backbone()`）：
...
数据由 **`src/simulation/pusch.py`**（SionnaPUSCHSystem）基于 [Sionna 2.x](https://nvlabs.github.io/sionna/phy/index.html)（PyTorch 后端）在线生成，实现**标准的 5G NR PUSCH 链路**（对齐 Sionna `sionna.phy.nr` 模块）：
...
```
llr_project/
├── configs/                 # JSON 配置：仿真/模型/训练/实验/路径（LLR_CFG=night 加载覆盖）
├── data/                    # Sionna 数据缓存（分片 pkl，训练复用，git 忽略）
├── src/                     # 源代码
│   ├── models/              # LWM 骨干（lwm.py）+ CNN 残差 LLR Decoder（llr_decoder.py）
│   │                        #   + 组合模型 LWMLLR（lwm_llr.py）
│   ├── datasets/            # 数据生成（generator 接口）/ demap 工具 / tokenizer / 分片加载
│   ├── trainers/            # 阶段1 MCM 预训练（pretrain.py）、阶段2 LLR 微调（train_llr.py）
│   ├── simulation/          # Sionna PUSCH 链路仿真（pusch.py）
│   ├── evaluation/          # 性能评估（evaluate.py）/ NeuralReceiver 实验（experiment.py）
│   │                        #   / 结果分析（analyze.py）
│   └── utils/               # 配置加载器（config.py，读 configs/*.json）
├── scripts/                 # 一键运行脚本（train_all.sh 默认 / train_night.sh 夜间大规模）
├── tests/                   # 单元测试（unittest）
├── notebooks/               # Jupyter Notebook（Sionna Neural Receiver 教程）
├── experiments/             # 实验输出
│   ├── logs/                # 训练/仿真日志
│   ├── checkpoints/         # 模型权重（lwm_continued.pt / lwm_llr.pt / lwm_llr_no_pretrain.pt）
│   └── results/             # 评估结果（eval_results.json / eval_ber_curves.png / 实验产物）
└── docs/                    # 项目文档（README.md 完整版 / REPORT.md 开发报告）
```
...
```bash
python -m src.trainers.pretrain
# 可选参数：
...
**产出**：`experiments/checkpoints/lwm_continued.pt`；数据缓存 `data/pusch_pt.pkl`
...
```bash
# 主模型（使用阶段1继续预训练权重）
python -m src.trainers.train_llr

# 对照模型（使用官方权重，无继续预训练，用于消融对比）
python -m src.trainers.train_llr --no-pretrain
...
**产出**：`experiments/checkpoints/lwm_llr.pt`（主）、`experiments/checkpoints/lwm_llr_no_pretrain.pt`（对照）
...
> 本节为最新版结果：**Sionna 标准基线**（`LMMSEEqualizer` + `Demapper`，含 LS 估计误差修正）+ **cfg 9 维**（不含信道模型信息）。

### 8.1 硬判决 BER vs SNR（主指标，越低越好）

| SNR | 传统基线 | **LWM+CNN** | 改善 | 对照(noPT) |
|-----|---------|-------------|------|-----------|
| -5 dB | 0.3861 | 0.3717 | **-3.7%** | 0.3609 |
| 0 dB | 0.3078 | 0.2419 | **-21.4%** ✅ | 0.2468 |
| 5 dB | 0.2299 | 0.1712 | **-25.5%** ✅ | 0.1752 |
| 10 dB | 0.1612 | 0.0980 | **-39.2%** ✅ | 0.1046 |
| 15 dB | 0.0833 | 0.0250 | **-70.0%** ✅ | 0.0249 |
| 20 dB | 0.0586 | 0.0130 | **-77.8%** ✅ | 0.0154 |

### 8.2 按调制阶数（BER, 全 SNR 平均）

| 调制 | 传统基线 | **LWM+CNN** | 改善 | 说明 |
|------|---------|-------------|------|------|
| QPSK | 0.1234 | 0.1414 | **-14.6%** | 模型差于基线 |
| 16QAM | 0.1644 | 0.1877 | **-14.2%** | 模型差于基线 |
| 64QAM | 0.2535 | 0.1725 | **+32.0%** ✅ | 模型优势 |
| 256QAM | 0.2766 | 0.1122 | **+59.5%** ✅ | 模型优势 |

> **调制规律（基线已确认无 bug）**：使用 Sionna 标准组件（LS+MMSE+maxlog）重建基线后结果不变——
> QPSK/16QAM 的逐 RE max-log LLR 已是**近似最优**（QPSK 精确），模型（小数据训练）无法超越反而引入误差；
> 64/256QAM 下逐 RE max-log 近似损失大（每比特涉及多个星座点），模型学习的**帧级联合后验**优势显著。
> 即"64/256QAM 好太多"是模型真实能力，非基线缺陷。

### 8.3 按配置维度（BER, 全 SNR 平均，lwm 相对 base 改善率）

| 维度 | 改善率 | 结论 |
|------|--------|------|
| 天线 1/2/4/8 | 25.5% / 13.0% / 23.0% / 40.9% | 全部改善，8 天线最优 |
| RB 1~10 | 11.9%~39.1% | 全部改善（6RB 最优 39.1%） |
| 符号 3~14 | 15.9%~39.1% | 全部改善 |
| DMRS {1}/{1+1}/{1+2} | 24.3% / 28.0% / 21.9% | 全部改善 |
| 速度 0/5/30 m/s | 21.3% / 24.0% / 29.8% | 全部改善（含 350Hz 多普勒） |

### 8.4 结论

- **一个模型适配所有系统参数**：天线 1/2/4/8 × RB 1~10 × 符号 3~14 × DMRS 三模式 × TDL-A/B/C/D × 时延/多普勒，-5~20 dB 全区间 BER 改善 **3.7%~77.8%**（高 SNR 段），除低阶调制外所有配置维度无劣化。
- **调制维度是唯一弱点**：QPSK/16QAM 模型差于基线（-14%）。根源是低阶调制下 max-log 已近最优、模型难以学得额外增益（且小数据下引入误差）。若面向低阶调制部署，可考虑混合损失/更多低阶数据。
- **两阶段训练 + 浅层特征**：冻结骨干训 decoder → 骨干 lr=1e-4 联合微调，CNN Decoder 输入含 LWM 第 3~6 层多尺度特征；中高 SNR（10~20dB）BER 相对早期版本显著下降。
- **cfg 只含接收端可感知参数**（9 维：天线/RB/符号/DMRS），信道模型信息（TDL/多普勒）不入模型输入（接收端无法感知），由模型从 H 估计中隐式学习。
- **继续预训练 vs 官方权重**：两者性能接近（主模型 15~20dB 略优）——骨干大 lr 微调覆盖 MCM 初始化差异；见 §11。
...
- 若报 `CUDA out of memory`：6GB 显存需 `--batch 16 --grad-accum 4`（见 configs/training.json 默认值）

**Q1c：Sionna 数据生成报 CPU/GPU 设备混合错误？**
Sionna 检测到 CUDA 会自动把全局 device 设为 cuda，导致其内部组件 CPU/GPU 混合。本项目已在 `src/simulation/pusch.py` 顶部强制 `sionna.config.device = "cpu"`（数据生成在 CPU，训练在 GPU），无需处理。

**Q2：官方权重路径报错怎么办？**
`configs/paths.json` 的 `LWM_CKPT_FILENAME` + `LWM_REPO_DIR_REL` 决定官方权重路径（默认 `../LWM/model_weights.pth`）。确认已克隆 LWM 仓库并下载真实权重（注意 git-lfs 指针问题，见 3.3 节）。
...
**Q5：如何用我自己的数据？**
修改 `src/simulation/pusch.py` 的 `SionnaPUSCHSystem`（信道模型、带宽、DMRS 配置等），或改写 `generate_dataset()` 读取自己的数据集。样本需保证字段一致（见 5.3 表）：`H_est(8,120,14)`、`z(1440,)`、`sigma2`、`llr_ref(1440,log2M)`、`bits_tx`、`mod_order`、`sigma2_eq`。

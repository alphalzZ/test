# -*- coding: utf-8 -*-
"""
阶段 1：MCM 继续预训练（设计文档第 5 节）
用自己的 3GPP OFDM 信道数据对 LWM 做 Masked Channel Modeling 域适配。

用法：python train_pretrain.py [--epochs 15] [--samples 3000] [--quick]
"""
import argparse
import os
import sys
...
from tokenizer import make_mcm_batch, tokenize_blocks
from model import load_official_backbone


def build_pretrain_data(n_samples, seed):
    print(f"[PT] 生成 {n_samples} 个 3GPP OFDM 信道样本 (seed={seed}) ...")
    t0 = time.time()
    samples = generate_dataset(n_samples, n_sc=config.N_SC, seed=seed)
    H = np.stack([s["H_est"] for s in samples]).astype(np.complex64)  # (N, 8, 128)
...
def train(args):
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        n_iter = 0
        for H_b in loader:
            H_b = H_b.numpy()                     # (B, 8, 128) complex
            blocks = []
            for i in range(H_b.shape[0]):
                blk, _ = tokenize_blocks(H_b[i])
                blocks.append(blk[0])
